...
const FTS_EXCEPTIONAL_CATEGORY='Liste déroulante — Règlement exceptionnel';
function seasonShopOptionKey(productId){return String(productId||'').replace(/[.#$\[\]\/]/g,'_');}
function cleanLegacySeasonShopMarkers(text){return String(text||'').split(/\n+/).filter(line=>{const value=line.trim();return value!=='__FTS_SAISON_ON__'&&value!=='__FTS_SAISON_OFF__';}).join('\n').trim();}
function seasonShopOptionEnabled(product,options){const key=seasonShopOptionKey(product&&product.id);if(key&&Object.prototype.hasOwnProperty.call(options||{},key))return options[key]===true;return String(product&&product.category||'')!==FTS_EXCEPTIONAL_CATEGORY;}
async function loadCartProducts(){if(ftsCartProductsLoaded)return;ftsCartProductsLoaded=true;try{const db=FTS.initFirebase();const configOptionPromise=db?db.ref('fts_saison/config/shopOptions').once('value').then(s=>s.val()||{}).catch(e=>{console.warn('[FTS panier] réglages Saison config indisponibles',e);return{};}):Promise.resolve({});const legacyOptionPromise=db?db.ref('fts_saison/shopOptions').once('value').then(s=>s.val()||{}).catch(()=>({})):Promise.resolve({});const [res,configOptions,legacyOptions]=await Promise.all([fetch(ftsPaymentApiBase()+'/catalog/products',{method:'GET',headers:{'Content-Type':'application/json'}}),configOptionPromise,legacyOptionPromise]);const data=await res.json().catch(()=>({}));const options=Object.assign({},legacyOptions||{},configOptions||{});const source=(Array.isArray(data.products)?data.products:[]).filter(p=>!String(p&&p.category||'').startsWith('__PIECES__:'));ftsCartProducts=source.filter(p=>seasonShopOptionEnabled(p,options)).map(p=>Object.assign({},p,{variantsText:cleanLegacySeasonShopMarkers(p&&p.variantsText)}));}catch(e){console.warn('[FTS panier] produits boutique indisponibles',e);ftsCartProducts=[];}}
