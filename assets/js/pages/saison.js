...
const FTS_SEASON_OPTION_ON_MARKER='__FTS_SAISON_ON__';
const FTS_SEASON_OPTION_OFF_MARKER='__FTS_SAISON_OFF__';
const FTS_EXCEPTIONAL_CATEGORY='Liste déroulante — Règlement exceptionnel';
function seasonShopOptionState(product){const lines=String(product&&product.variantsText||'').split(/\n+/).map(line=>line.trim());if(lines.includes(FTS_SEASON_OPTION_ON_MARKER))return 'on';if(lines.includes(FTS_SEASON_OPTION_OFF_MARKER))return 'off';return String(product&&product.category||'')===FTS_EXCEPTIONAL_CATEGORY?'off':'legacy';}
function cleanSeasonShopMarkers(text){return String(text||'').split(/\n+/).filter(line=>{const value=line.trim();return value!==FTS_SEASON_OPTION_ON_MARKER&&value!==FTS_SEASON_OPTION_OFF_MARKER;}).join('\n').trim();}
async function loadCartProducts(){if(ftsCartProductsLoaded)return;ftsCartProductsLoaded=true;try{const res=await fetch(ftsPaymentApiBase()+'/catalog/products',{method:'GET',headers:{'Content-Type':'application/json'}});const data=await res.json().catch(()=>({}));const source=(Array.isArray(data.products)?data.products:[]).filter(p=>!String(p&&p.category||'').startsWith('__PIECES__:'));const explicit=source.filter(p=>seasonShopOptionState(p)==='on');const legacy=source.filter(p=>seasonShopOptionState(p)==='legacy');ftsCartProducts=explicit.concat(legacy).map(p=>Object.assign({},p,{variantsText:cleanSeasonShopMarkers(p&&p.variantsText)}));}catch(e){console.warn('[FTS panier] produits boutique indisponibles',e);ftsCartProducts=[];}}
