...
async function loadCartProducts(){if(ftsCartProductsLoaded)return;ftsCartProductsLoaded=true;try{const res=await fetch(ftsPaymentApiBase()+'/catalog/products',{method:'GET',headers:{'Content-Type':'application/json'}});const data=await res.json().catch(()=>({}));ftsCartProducts=(Array.isArray(data.products)?data.products:[]).filter(p=>!String(p&&p.category||'').startsWith('__PIECES__:'));}catch(e){console.warn('[FTS panier] produits boutique indisponibles',e);ftsCartProducts=[];}}
