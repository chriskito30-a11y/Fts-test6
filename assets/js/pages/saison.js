...
async function loadCartProducts(){if(ftsCartProductsLoaded)return;ftsCartProductsLoaded=true;try{const db=FTS.initFirebase();const optionPromise=db?db.ref('fts_saison/shopOptions').once('value').then(s=>s.val()||{}).catch(e=>{console.warn('[FTS panier] réglages Saison indisponibles',e);return{};}):Promise.resolve({});const [res,options]=await Promise.all([fetch(ftsPaymentApiBase()+'/catalog/products',{method:'GET',headers:{'Content-Type':'application/json'}}),optionPromise]);const data=await res.json().catch(()=>({}));const source=(Array.isArray(data.products)?data.products:[]).filter(p=>!String(p&&p.category||'').startsWith('__PIECES__:'));ftsCartProducts=source.filter(p=>seasonShopOptionEnabled(p,options)).map(p=>Object.assign({},p,{variantsText:cleanLegacySeasonShopMarkers(p&&p.variantsText)}));}catch(e){console.warn('[FTS panier] produits boutique indisponibles',e);ftsCartProducts=[];}}
