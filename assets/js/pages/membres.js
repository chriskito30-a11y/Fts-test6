...
function memberEventPriceCents(e){
  e=e || {};
  for(const key of ['memberPriceCents','priceMemberCents','tarifMembreCents']){
    if(e[key] !== undefined && e[key] !== null && e[key] !== '') return Math.max(0, Number(e[key]) || 0);
  }
...
function showDocs(docs, idx) {
  ...
  el.innerHTML = docs.map(d => {
    ...
    const rehearseAction = isRehearsalPdfDoc(d)
      ...
      : '';
    return `<div class="doc-file-row doc-file-row--${FTS.esc(kind.cls)}${rehearseAction ? ' doc-file-row--rehearsal' : ''}" id="doc-${FTS.esc(d.key || '')}" data-doc-key="${FTS.esc(d.key || '')}">
